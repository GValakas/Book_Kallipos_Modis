...
install.packages("gstats") # Install the package gstat
install.packages("plot3D") # Install the package plot3D
install.packages("plot3Drgl") # Install the package plot3Drgl
...
SouthFieldDataShort<-read.csv("C:/Users/georv/Documents/GV Workspace/Κάλλιπος 2020/Ενδιάμεση Αναφορά 2021/Κώδικες -Δεδομένα Βιβλίου στην R Ανεβασμένοι στο Helios/SouthFieldDataShort.txt", header = TRUE, sep = "", quote = "\"'")
...
plot(data_unique_id$x,data_unique_id$y, 
main="Κάτοψη Γεωτρήσεων", xlab=" x (m)", ylab="y (m)", 
pch = 22, col="blue", xlim=c(-15000,-12000), ylim=c(17000,20000))
...
for (i in 1:nrow(SouthFieldDataShort)){
	z_transf<-SouthFieldDataShort$z[i] - seq(SouthFieldDataShort$from[i], SouthFieldDataShort$to[i], z_step) # δημιοργία μετασχηματισμένου z για την i γραμμή
...
library(plot3D)
library(plot3Drgl)
scatter3D(x = data_field$x, y = data_field$y, 
	z = data_field$z_transf, colvar = data_field$lithological_code, 
    col = c("blue", "green", "cyan","orange","red"), 
	...
	expand = 0.1,
    labels = c("SG", "MG", "CO","AL","HD"),cex.axis = 1))
plotrgl(new = TRUE)  
